;(function (exports, DOMParser, xmlbuilder) {
	...
	// if in a browser, find the DOMParser
	if (!inNode) {
	...
	exports.parseString = function (xml, callback) {
		...
		try {
			...
			json_obj = parsePlistXML(doc.documentElement);
		} catch(e) {
			error = e;
		}
		callback(error, json_obj);
	}
	 ...
	 * @return build up JSON object
	 */
	function parsePlistXML(node) {
		var i, new_obj, key, val, new_arr;
		if (!node)
			return null;
		//console.log('name:', node.nodeName, node.nodeType, node.localName);
		if (node.nodeName === 'plist') {
			...
			key = null;
			for (i=0;i < node.childNodes.length;i++) {
				//console.log('here', node.childNodes[i].nodeType, node.childNodes[i].nodeValue);
				// ignore comment nodes (text) 
				if (node.childNodes[i].nodeType !== 3) {
					if (key === null) {
			...
			for (var d=0; d < node.childNodes.length; d++)
			{
				...
				//console.log ('yepp', node.childNodes[d].nodeValue, node.childNodes[d].nodeName, node.childNodes[d].nodeType);
			}
			return res;
		}
