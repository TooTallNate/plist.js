              ...
              if (typeof o[name]!== 'function') {
                  if (typeof o[name]=== 'object') {
                      if (o[name] instanceof Date) {
                          // FIXME format date correctly.
                          child = child.ele('key').txt(name).up().ele('date').txt(o[name]).up();
                      } else {
                          if (o[name] instanceof Array) {
                                child = child.ele('array'); 
                            }  else {
                      ...
                      child.ele('key').txt(name);
                      if ((typeof o[name]) === 'boolean') {
                          child.ele(o[name]);
                      } else if ((typeof o[name]) === 'number') {
                          // FIXME detect and handle real
                          child.ele('integer').txt(o[name]);
                      } else {
                          // FIXME detect base64 and do data instead
                          child.ele('string').txt(o[name]);
                      }
                  }
              }
          }
