;(function (exports, sax) {
 //Checks if running in a non-browser environment
  ...
  exports.stringify = function(obj) {
    ...
    // Type detections from underscore.js
    if (toString.call(obj) == '[object Number]') {
      
      //todo detect integers ?
      //return "<integer>"+obj.toString()+"</integer>";

      return "\n<real>"+obj.toString()+"</real>";

    } else if (toString.call(obj) == '[object Date]') {

      return "\n<date>"+obj.getUTCFullYear()+"-"+(obj.getUTCMonth()+1)+"-"+obj.getUTCDate()+"T"+obj.getUTCHours()+":"+obj.getUTCMinutes()+":"+obj.getUTCSeconds()+"Z</date>";

    } else if (obj === true || obj === false || toString.call(obj) == '[object Boolean]') {
      
      return obj?"\n<true/>":"\n<false/>";

    } else if (toString.call(obj) == '[object String]') {

      var str = encodeXML(obj);
    ...
    //Object
      ...
      for(var prop in obj) {
        if(obj.hasOwnProperty(prop)) {
          r+="\n<key>"+encodeXML(prop)+"</key>"+exports.stringify(obj[prop]); 
        }
      }
